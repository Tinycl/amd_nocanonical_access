...
int main(int argc, char **argv) {
  unsigned long leak_addr, victim_addr;
  unsigned long leak_len = 16;
 
  victim_addr = (unsigned long)secret;
  printf("secret addr is 0x%x, data is %s\n", victim_addr,secret);
 
 	printf("start to leak:\n");	
  leak_addr = victim_addr | 0xff0f000000000000;
  hexdump_memory(leak_addr, leak_len);
}
